...
LR = 1e-4 # transformer should have a smaller lr than cnn
...
class CropSegDataset(Dataset):
    ...
    def _augment(self, image, mask):
        ...
        if torch.rand(1).item() > 0.5:
            brightness_factor = float(torch.empty(1).uniform_(0.9, 1.1).item())
            contrast_factor = float(torch.empty(1).uniform_(0.9, 1.1).item())
...
def evaluate(model, loader, criterion):
    ...
    for images, masks in loader:
        ...
        # convert to uint8 numpy for compute_metrics
        for pred, mask in zip(preds, masks):
            pred_np = (pred.numpy() * 255).astype(np.uint8)
